...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        String progName = this.name();

        out.println("Program " + progName + " IS");
        out.println();
        while (ctxt.size() > 0) {
            Map.Pair<String, Statement> temp = ctxt.removeAny();
            String name = temp.key();
            Statement body = temp.value();

            printSpaces(out, INDENT_SIZE);
            out.println("INSTRUCTION " + name + " IS");

            body.prettyPrint(out, INDENT_SIZE * 2);
        ...
        tempBody.prettyPrint(out, INDENT_SIZE);
        this.swapBody(tempBody);
        out.print("END " + progName);
    }
