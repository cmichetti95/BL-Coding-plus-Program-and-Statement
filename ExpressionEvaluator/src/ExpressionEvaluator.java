...
public final class ExpressionEvaluator {
    ...
    private static int valueOfDigit(StringBuilder source) {
        ...
        int finalVal = Character.digit(source.charAt(0), RADIX);
    ...
    private static int valueOfDigitSeq(StringBuilder source) {
        ...
        while (Character.isDigit(source.charAt(0))) {
            finalVal = valueOfDigit(source);
            number += Integer.toString(finalVal);
        }
        finalVal = Integer.parseInt(number);
        return finalVal;
    }
    ...
    private static int valueOfFactor(StringBuilder source) {
        ...
        if (source.charAt(0) == '(') {
            source.deleteCharAt(0);
            finalVal = valueOfExpr(source);
            source.deleteCharAt(0);
        } else {
            finalVal = valueOfDigitSeq(source);
        }
        return finalVal;
    }
    ...
    private static int valueOfTerm(StringBuilder source) {
        ...
        int finalVal = valueOfFactor(source);
        while (source.charAt(0) == '/' || source.charAt(0) == '*') {
            char divOrMult = source.charAt(0);
            source.deleteCharAt(0);
            if (divOrMult == '/') {
                finalVal /= valueOfFactor(source);
            } else {
                finalVal *= valueOfFactor(source);
            }
        }
    ...
    public static int valueOfExpr(StringBuilder source) {
        ...
        int finalVal = valueOfTerm(source);
        while (source.charAt(0) == '-' || source.charAt(0) == '+') {
            char minOrPlus = source.charAt(0);
            source.deleteCharAt(0);
            if (minOrPlus == '-') {
                finalVal -= valueOfTerm(source);
            } else {
                finalVal += valueOfTerm(source);
            }
        }
