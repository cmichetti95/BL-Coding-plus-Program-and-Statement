...
public final class Program1GeneratedCode1 extends Program1 {
        ...
        switch (s.kind()) {
            ...
            case CALL: {
                ...
                if (context.hasKey(instr)) {
                    generateCodeForStatement(context.value(instr),
                            context.newInstance(), cp);
                } else {
                    instr = instr.toUpperCase();
                    cp.add(cp.length(), Instruction.valueOf(instr).byteCode());
    ...
    public Sequence<Integer> generatedCode() {
        ...
        generateCodeForStatement(newBody, ctxt, cp);
